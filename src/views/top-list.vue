      ...
      <ul>
        ...
        >
          ...
          <ul class="song-list">
            <li
              class="song"
              v-for="(song, index) in item.songs?.slice(0,3)"
              :key="song.id">
              <span>{{index + 1}}. </span>
              <span>{{song.name}}-{{song.ar.map(artist => artist.name).join('/')}}</span>
            </li>
          </ul>
        </li>
      </ul>
...
import { getAlbumDetail } from '@/service/recommend'
  ...
  export default {
    ...
    async created() {
      ...
      await Promise.all(this.topList.map(async (item) => {
        const songs = await getAlbumDetail(item.id)
        item.songs = songs.playlist.tracks
